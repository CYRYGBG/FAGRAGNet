...
class MultiHeadSelfAttention(nn.Module):
	def __init__(self, dim_in, dim_out, channel, dim_k=8*64, dim_v=8*64, num_heads=8, dropout=0.):
		...
		self.to_out = nn.Sequential(
			nn.Linear(channel * dim_k, dim_out),
			nn.Dropout(dropout)
		)
...
class FreqBandsAttention(nn.Module):
	def __init__(self, n_channels, hidden_size):
		super().__init__()
		self.linear_in = MultiHeadSelfAttention(dim_in=64, dim_out=5, channel=5)
...
class FAGRAGNet(nn.Module):
	def __init__(self):
		...
		self.FBA = FreqBandsAttention(n_channels=5, hidden_size=32)

		self.conv1 = Conv2d(1, 32, (2, 2), padding=(3 // 2, 3 // 2))
		self.drop1 = Dropout(drop_rate)
		self.RG1 = ResidualGraph(65 * 32 * 6, 64, 32, topk)

		self.conv2 = Conv2d(32, 32, (2, 2), padding=(3 // 2, 3 // 2))
		self.drop2 = Dropout(drop_rate)
		self.RG2 = ResidualGraph(66 * 32 * 7, 64, 32, topk)

		self.conv3 = Conv2d(32, 32, (1, 3))
		self.drop3 = Dropout(drop_rate)
		self.pool3 = MaxPool2d((1, 2))
		self.RG3 = ResidualGraph(32 * 32 * 7, 64, 32, topk)

		self.conv4 = Conv2d(32, 64, (1, 3))
		self.drop4 = Dropout(drop_rate)
		self.pool4 = MaxPool2d((1, 2))
		self.RG4 = ResidualGraph(7 * 15 * 64, 64, 32, topk)

		self.conv5 = Conv2d(64, 128, (1, 2))
		self.drop5 = Dropout(drop_rate)
		# self.pool3 = MaxPool2d((1, 4))
		self.RG5 = ResidualGraph(14 * 7 * 128, 64, 32, topk)

		self.drop6 = Dropout(drop_rate)
		...
		self.linend2 = Linear(1024, 4)


	def forward(self, x):
		# input shape: [batch*channel, 1, Freq_bands, Features]
		x = x.reshape(-1, 1, 5, 64)  # (Batch*channels, 1, Freq_bands, Features)
